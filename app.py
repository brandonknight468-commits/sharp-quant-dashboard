...
# ==============================================================================
# 2. HELPER CALCULATIONS (CRASH-PROOF)
# ==============================================================================
def american_to_decimal(american):
    if american is None or american == 0:
        return 1.0  # Safe fallback to prevent ZeroDivisionError
    if american > 0:
        return (american / 100.0) + 1.0
    else:
        return (100.0 / abs(american)) + 1.0

def parse_commence_time(time_str):
    if not time_str:
        return None
    try:
...
st.markdown("This model strictly filters out live games and invalid odds feeds.")
            ...
            try:
                ...
                for sport, data in raw_results:
                    for event in data:
                        ...
                        game_start = parse_commence_time(commence_time)
                        if game_start and game_start < datetime.now(timezone.utc):
                            continue

                        matchup = f"{event.get('away_team')} @ {event.get('home_team')}"
                        bookies = {b['key']: b for b in event.get('bookmakers', []) if isinstance(b, dict)}
                        ...
                        for book_key, book in bookies.items():
                            ...
                            for market in book.get('markets', []):
                                ...
                                for g_key, outs in groups.items():
                                    if len(outs) == 2:
                                        try:
                                            price_0 = outs[0].get('price', 0)
                                            price_1 = outs[1].get('price', 0)
                                            if price_0 == 0 or price_1 == 0: continue
                                            
                                            implied_a = 1.0 / american_to_decimal(price_0)
                                            implied_b = 1.0 / american_to_decimal(price_1)
                                        except Exception: 
                                            continue
                                        ...
                                        b_key_a = (m_key, outs[0].get('name'), outs[0].get('description', 'Game'), outs[0].get('point'))
                                        b_key_b = (m_key, outs[1].get('name'), outs[1].get('description', 'Game'), outs[1].get('point'))
                        ...
                        for book_key, book in bookies.items():
                            ...
                            for market in book.get('markets', []):
                                ...
                                for out in market.get('outcomes', []):
                                    bet_key = (m_key, out.get('name'), out.get('description', 'Game'), out.get('point'))
                                    
                                    if bet_key in true_probs:
                                        true_p = true_probs[bet_key]
                                        soft_odds = out.get('price', 0)
                                        if soft_odds == 0: continue
                                        ...
                                        if tier:
                                            ...
                                            ev_opportunities.append({
                                                ...
                                                "Soft Book": book.get('title', '').upper(),
                ...
                if ev_opportunities:
                    ...
                    st.info("No qualifying PRE-GAME edges currently available on the selected sports. Check back during the optimal scan windows!")

            except Exception as e:
                st.error(f"Scanner encountered an issue parsing the odds feed: {e}")
